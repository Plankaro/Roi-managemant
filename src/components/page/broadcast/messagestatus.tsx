...
export default function MessageStatus({
  ...
  const { readCount, failedCount, skippedCount, deliveredCount } =
    selectedBroadcast;
      ...
      <CardContent className="space-y-8 md:p-6 p-1">
        ...
        <Tabs defaultValue="failed" className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-transparent">
            <TabsTrigger value="failed" className="text-xs sm:text-base">
              Failed Reasons
            </TabsTrigger>
            <TabsTrigger value="skipped" className="text-xs sm:text-base">
              Skipped Reasons
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="failed">
            <ScrollArea className="h-[200px] w-full border-none p-4 no-scrollbar">
              {selectedBroadcast?.failedReasonGroups?.length ? (
                <div className="space-y-3">
                  {selectedBroadcast.failedReasonGroups.map((item, index) => (
                    <div
                      key={index}
          ...
          <TabsContent value="skipped">
            <ScrollArea className="h-[200px] w-full border-none p-4 no-scrollbar">
              {selectedBroadcast?.skippedReasonGroups?.length ? (
                <div className="space-y-3">
                  {selectedBroadcast.skippedReasonGroups.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-start justify-between gap-4"
                    >
