...
export default function ProspectList({prospects}: {prospects:CustomerProspectList}) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {prospects && prospects?.customers?.map((prospect: CustomerProspect) => (
          <Link href={`/prospects/${prospect?.shopifyCustomer?.shopify_id}`} key={prospect.shopifyCustomer.shopify_id}>
              ...
              <div className="relative">
                <Avatar className="h-16 w-16">
                  <AvatarImage
                    ...
                    alt={prospect?.prospectData?.name || prospect?.shopifyCustomer?.name || ""}
                  />
                  <AvatarFallback>{prospect.prospectData?.name.slice(0, 2).toUpperCase() || prospect?.shopifyCustomer?.name.slice(0, 2).toUpperCase() || ""}</AvatarFallback>
                </Avatar>
                <IoLogoWhatsapp fill={"#10F600"} className="absolute bottom-0 right-0" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between">
                  <p className="text-sm font-medium truncate">{prospect?.prospectData?.name || prospect?.shopifyCustomer?.name|| prospect?.shopifyCustomer?.phone || prospect.prospectData?.phoneNo || ""}</p>
                  <button
                
                    className="h-5 w-5 text-gray-400"
                    // onClick={() => toggleStar(prospect.id)}
                  >
                    {/* fill={starredProspects.has(prospect.id) ? "currentColor" : "none"} */}
                    <Star className="h-4 w-4 hover:fill-gray-400"  />
                  </button>
                </div>
                <div className="mt-2 flex items-center gap-4 text-xs text-gray-400">
                  ...
                  <span className="bg-[#4166AE24] py-2 px-2">{prospect?.prospectData?.last_Online || "no online status found"}</span>
                </div>
              </div>
            </div>
          </Card>
