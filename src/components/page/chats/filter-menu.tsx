    ...
    return (
        <div className="bg-transparent text-white p-4">
            <div className="flex items-center space-x-2">
                <Button
                    variant="link"
                    className="text-red-500 hover:text-red-400 px-0"
                    onClick={clearAllFilters}
                >
                    Clear All
                </Button>
                <ScrollArea className="w-full max-h-[400px] overflow-x-auto"> {/* Enable horizontal scroll */}
                    <div className="flex gap-2"> {/* Make sure the filters are laid out horizontally */}
                        {
                            filters?.map((filter) => (
                                <DropdownMenu key={filter.title}>
                                    <DropdownMenuTrigger asChild>
                                        <Button
                                            variant={"ghost"}
                                            className="flex items-center hover:bg-slate-800 h-auto hover:bg-transparent hover:text-white/90"
                                        >
