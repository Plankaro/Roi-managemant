        ...
        <div className="flex md:items-center gap-3 md:flex-row flex-col justify-between mb-6">
          ...
          <div className="flex md:items-center md:flex-row flex-col gap-3">
            ...
            <FlashResponseCreateModal onSave={handleAddResponse}>
              <Button className="bg-blue-500 hover:bg-blue-600 text-white h-9 rounded-md flex items-center gap-1 px-4">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-scroll max-h-[calc(100vh-200px)] no-scrollbar">
            {Array(6)
              .fill(0)
              .map((_, index) => (
                <div key={index} className="bg-backgroundColor border relative h-[250px] border-primary rounded-lg p-3">
                  <div className="p-4">
                    <div className="mb-2">
                      <Skeleton className="h-8 w-3/4 bg-indigo-900/50 mb-2" />
                      <Skeleton className="h-4 w-1/3 bg-indigo-900/50" />
                    </div>
                    <Skeleton className="h-[70px] w-full bg-indigo-900/50 mb-4" />
                    <div className="mt-4 border-t border-[#D9D9D933] py-2">
                      <div className="flex justify-between items-center">
                        <div className="flex flex-col gap-1">
                          <Skeleton className="h-4 w-20 bg-indigo-900/50" />
                          <Skeleton className="h-4 w-24 bg-indigo-900/50" />
                        </div>
                        <div className="flex space-x-1">
                          <Skeleton className="h-6 w-6 rounded-full bg-indigo-900/50" />
                          <Skeleton className="h-6 w-6 rounded-full bg-indigo-900/50" />
                        </div>
                      </div>
                    </div>
          ...
          // Empty state when no flash responses exist
          <div className="bottom-full left-0 right-0 rounded-t-lg h-[60vh] p-8 flex flex-col items-center justify-center">
            ...
            <p className="text-center text-gray-300 mb-6 max-w-md">
              It looks like you haven&apos;t created any Flash Responses yet. Flash Responses help you send
              pre-configured messages instantly, making conversations faster and more efficient. Start by clicking
              &apos;New Response&apos; and set up your first quick reply now!
            </p>
            <FlashResponseCreateModal onSave={handleAddResponse}>
              <Button className="bg-blue-500 hover:bg-blue-600 text-white">+ Create Flash Response</Button>
            </FlashResponseCreateModal>
          </div>
        ) : (
          <div>
            {filteredResponses?.length === 0 ? (
              ...
              <div className="bottom-full left-0 right-0 rounded-t-lg h-[60vh] p-8 flex flex-col items-center justify-center">
                ...
                <p className="text-center text-gray-300 mb-6 max-w-md">
                  We couldn&apos;t find any flash responses matching &quot;{searchQuery}&quot;
                </p>
                {/* <Button
                  variant="outline"
              ...
              // Display filtered flash responses
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-scroll max-h-[calc(100vh-200px)] no-scrollbar">
                {filteredResponses?.map((response: any) => (
                  ...
                  >
                    <div className="p-4">
                      <div className="mb-2">
                        <div className="text-white font-medium text-2xl">{response.heading}</div>
                        <div className="text-xs text-gray-400">Category: {response.category}</div>
                      </div>
                      <p className="text-white text-sm mb-4 line-clamp-3 h-[70px]">{response.message}</p>
                      <div className="">
                        <div className="flex justify-between items-center mt-4 border-t border-[#D9D9D933] py-2">
                          ...
                          <div className="flex space-x-1">
                            <FlashResponseEditModal response={response} onSave={handleEditResponse}>
